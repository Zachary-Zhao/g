...
func Run() {
	...
	app.Before = func(ctx *cli.Context) (err error) {
		ghomeDir = ghome()
		fmt.Println("数据目录：",ghomeDir)
		goroot = filepath.Join(ghomeDir, "go")
		downloadsDir = filepath.Join(ghomeDir, "dls")
		if err = os.MkdirAll(downloadsDir, 0755); err != nil {
			return err
		}
		versionsDir = filepath.Join(ghomeDir, "vns")
...
func ghome() (dir string) {
	var homeDir string
	if homeDir = os.Getenv("G_BASE"); homeDir == "" {
...
func render(curV string, items []*semver.Version, out io.Writer) {
	...
	for i := range items {
		...
		if v == curV {
			color.New(color.FgHiGreen, color.Bold).Fprintf(out, "* %s\n", v)
		} else {
			fmt.Fprintf(out, "  %s\n", v)
		}
	}
